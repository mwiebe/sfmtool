...
def grow_reconstruction(
    ...
    tvec = np.tile([0.0, 0.0, -f0], (n_img, 1))  # canonical: -Z is in front
...
def perspective_init(rot, scale, t_cam, used, f0):
    ...
    (-Z forward, +Y up).  The point centroid sits at depth -f0/s with lateral
    offset (t/s); the factorization emits COLMAP-frame (+Z forward) rotations,
    so both the rotations and translations are mapped by S = diag(1, -1, -1)
    (negate the y/z rows / components)."""
...
def project(cam, rvec, tvec, pts, obs_i, obs_c):
    """Full-pixel reprojection and in-front depth of each observation.

    Canonical camera-space points ``R·X + t`` go to full pixels via
    ``ray_to_pixel_batch`` (NaN for obs behind the camera); the returned depth
    is ``-z`` so it is positive in front (canonical z is negative in front)."""
...
def solve_round(obs_c, obs_i, u, f, rvec, tvec, pts, n_img, f_scale, opt_f):
    ...
    oi2_u32 = oi2.astype(np.uint32)
    oc2_u32 = oc2.astype(np.uint32)
    ...
    def resid(x):
        ...
        # Native reprojection residuals (proj − obs); behind-camera obs get a
        # 1e6 outlier residual (soft_l1 saturates them) — mirrors the old
        # max(z, 1e-6) clamp.
        return _reprojection_residuals(
            cam,
...
def compare_to_reference(names, rvec, tvec, f_est, mask=None):
    ...
    # Similarity alignment (est centers -> ref centers) via the native scaled
    # Kabsch; r_al feeds the rotation-error comparison, the transform maps the
    # centers for the center error.
    from sfmtool._sfmtool.analysis import estimate_alignment_rs
    ...
    qd = tf.to_dict()["rotation"]
    r_al = Rotation.from_quat([qd["x"], qd["y"], qd["z"], qd["w"]]).as_matrix()
