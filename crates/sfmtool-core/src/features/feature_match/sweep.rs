...
//!
//! Each of the four public entry points is a thin wrapper: the two one-way
//! sweeps over `match_one_way_sweep_inner`, the two bidirectional ones over
//! `mutual_best_match_sweep_inner`. The geometric variants differ only in
//! carrying per-feature affine shapes through the sort and narrowing each
//! window to the candidates passing the two-stage filter before any descriptor
//! is compared, so that difference is one `Option` parameter rather than a
//! parallel family of functions. [`super::polar`] is organized the same way.
...
/// The extra per-feature data the geometric path needs, already in sorted
/// order: the two sides' affine shapes and the filter to apply. Positions are
/// not carried here — the sweep already takes them, to slide the window.
...
/// The shared body of both one-way entry points.
///
/// `geometric` carries the only difference: `Some(..)` narrows each window to
/// the geometrically consistent candidates before comparing descriptors,
...
/// The geometric filter's inputs, gathered into sorted order once and held for
...
/// affines2, geom, config))` runs the geometric filter over each window, `None`
...
fn mutual_best_match_sweep_inner(
    ...
    let sorted_kpts1 = gather_rows(keypoints1, 2, &sort_idx1);
    let sorted_descs1 = gather_rows(descriptors1, desc_len, &sort_idx1);
    let sorted_kpts2 = gather_rows(keypoints2, 2, &sort_idx2);
    let sorted_descs2 = gather_rows(descriptors2, desc_len, &sort_idx2);

    let sorted_geometry = geometric.map(|(affines1, affines2, geom, config)| SortedGeometry {
        affines1: gather_rows(affines1, 4, &sort_idx1),
        affines2: gather_rows(affines2, 4, &sort_idx2),
        geom,
