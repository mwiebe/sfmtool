...
| `state/ops.rs` | `AppState`'s reconstruction *operations*, as a second `impl` block: `load_file`, `load_demo`, `align_node`, `with_resect_source` and `load_resect_matches`. These are the methods that read a file off disk or run a fit, build a `SceneNode` out of the answer and write the sentence the user reads about it. |
