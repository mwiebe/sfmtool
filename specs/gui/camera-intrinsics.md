...
without an egui context. That is
[`camera/report.rs`](../../crates/sfmtool-core/src/camera/report.rs), the
`camera::report` module:
...
- **Wiring the Python `_CAMERA_PARAM_NAMES` table to `parameter_names()`.**
  ...
  table has no entry for `EQUIRECTANGULAR`, `EQUIDISTANT_FISHEYE`,
  `SFMTOOL_FISHEYE` or `SFMTOOL_PINHOLE`, so for those four `sfm inspect` falls
  back to the map's lexicographic order — including the
  `bspline_c10`-before-`bspline_c2` ordering `parameter_names()` exists to avoid.
  The GUI and `sfm inspect` therefore do not yet agree glyph for glyph.
