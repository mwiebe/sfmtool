...
- **Campaign datasets** (picked from `test-data/` and `C:/DataSets` for
  ...
  1080p video frames — the existing April solve is .sfmr v1 with failed
  integrity, so a fresh subset solve is the reference), Kerry Park fisheye
...
- **Swivel_Chair subset (106 of 632 1080p video frames, stride 6, indoor
  object orbit), 12.7 min.** The existing April solve is .sfmr v1 with
  failed integrity — reference is a fresh incremental solve on the same
  subset. Sharp focal peak again (84.1% at 2688). 78.9% of 80,615
  observations < 2 px, 9,609 points; f 2709.9 vs 2740.4 (−1.1%). The most
  accurate cameras of the campaign: rotation mean **0.26°** / max 0.70°,
  centers mean 0.24% / max 0.60%. A dense orbit of a texture-rich object
  at video frame rate is the easy case, even indoors at 1080p.
...
| Swivel_Chair (subset) | 1080p video orbit | 106 | 80,615 | 78.9% | 0.26 / 0.70 | 0.2 / 0.6 | −1.1% | 13 min |
