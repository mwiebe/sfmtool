...
| [operation-progress.md](operation-progress.md) | [gui/action-log.md](../gui/action-log.md), [gui/mcp-server.md](../gui/mcp-server.md); adds a type to `sfmtool-core` | One `Progress` parameter every long `sfmtool-core` function takes, carrying phases, messages, counts and a cancel flag, because a rayon-parallel kernel cannot be timed by an ambient collector. In the viewer: a toggle at the start of an Action Log row that expands the operation's phases and messages in place, an `elsewhere` line reconciling them against the headline cost, and a detail level switchable from the window or over MCP. |
